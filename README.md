...
```text
请把 ask-vision 作为全局 Agent Skill 安装或更新到当前 Agent 的用户级 Skill 目录。Skill 来源为 https://github.com/Kangede/ask-vision；聚合索引仓库为 https://github.com/Kangede/agent-skills，并通过 Git submodule 引用该上游。请保留 SKILL.md、scripts、references 和 agents 目录；如果已安装同名 Skill，请更新现有安装，不要生成重复或多层嵌套目录。
...
- 来源 submodule：[skills/qwen-image-local](skills/qwen-image-local)（指向完整 `LocalLLMProxy` 仓库）
...
```text
请把 qwen-image-local 作为全局 Agent Skill 安装或更新到当前 Agent 的用户级 Skill 目录。Skill 来源为 https://github.com/Kangede/LocalLLMProxy/tree/main/skills/qwen-image-local；聚合索引仓库为 https://github.com/Kangede/agent-skills，并通过 Git submodule 引用 LocalLLMProxy。请完整保留 SKILL.md、scripts、references、config、assets 和 agents 目录；如果已安装同名 Skill，请更新现有安装，不要创建重复目录。
...
也可以让支持 Skill 安装器的 Agent 直接从各自的上游 GitHub 路径全局安装。`LocalLLMProxy` 当前为私有仓库，因此克隆聚合仓库的 Qwen submodule 或直接安装该 Skill 时，需要使用有权限的 GitHub 账号。安装后若未立即出现，请重启对应 Agent。
