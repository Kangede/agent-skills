...
description: Generate or edit raster images through LocalLLMProxy's Qwen-Image-2.1 endpoint when the active agent model is Qwen, GLM, or another non-GPT model. Do not use in GPT-family Codex sessions; use the built-in imagegen skill there. Supports text-to-image, up to five reference images, native PNG transparency, and project-local output files.
---

# Qwen Image Local

Use the project-local Qwen-Image-2.1 service to generate or edit bitmap images through an authenticated LocalLLMProxy Images API.
...
9. If the availability check fails, stop. Do not bypass LocalLLMProxy by calling the SGLang address directly, and do not repeatedly retry an unavailable model.
