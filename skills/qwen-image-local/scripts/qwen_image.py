#!/usr/bin/env python3
"""CLI for Qwen-Image-2.1 through LocalLLMProxy's OpenAI-compatible Images API."""
...
def discover_connection(args: argparse.Namespace, *, require_key: bool) -> Connection:
    ...
    if explicit_base:
        ...
        if codex:
            ...
            if match:
                ...
                source = base_name
                paired = {
                    "LOCAL_LLM_PROXY_BASE_URL": "LOCAL_LLM_PROXY_KEY",
    ...
    if not base_url:
        fail(
            "cannot discover the current LocalLLMProxy address; set QWEN_IMAGE_BASE_URL "
            "or pass --base-url"
        )
    ...
    if explicit_key_env:
        ...
        key_candidates = ["QWEN_IMAGE_API_KEY", "LOCAL_LLM_PROXY_KEY"]
    else:
...
def add_shared_connection(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--base-url", help="LocalLLMProxy base URL; auto-discovered when omitted")
